<x-app-layout>
    <x-slot name="title">{{ $style->is_system ? 'Tạo ảnh AI' : $style->name . ' - Studio' }} |
        {{ App\Models\Setting::get('site_name', 'ZDream') }}</x-slot>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        ...
        <div class="grid lg:grid-cols-5 gap-6 lg:gap-8">
            <!-- Left: Style Preview (2/5) - Hide for system styles -->
            @unless($style->is_system)
            ...
            <!-- Right: Generator (3/5 for normal, 5/5 for system) -->
            <div class="{{ $style->is_system ? 'lg:col-span-5' : 'lg:col-span-3' }}">
                @livewire('image-generator', ['style' => $style, 'initialPrompt' => $initialPrompt ?? ''])
            </div>
        </div>
    </div>
</x-app-layout>