                ...
                @else
                    ...
                    <div wire:key="editor-canvas-container"
                        ...
                        <div class="mb-6 bg-white/[0.05] backdrop-blur-xl rounded-2xl border border-white/[0.1] shadow-2xl p-2 sticky top-4 z-40 transition-all duration-300"
                            ...
                            <div class="flex flex-wrap items-center gap-2 md:gap-3 lg:gap-4 p-1">
                                
                                {{-- 1. Change Image --}}
                                <div class="order-1 md:order-1 flex-shrink-0">
                                    <label for="image-upload" 
                                           class="p-2 md:px-4 md:py-2.5 text-sm font-semibold text-white bg-gradient-to-r from-green-600 to-emerald-500 hover:from-green-500 hover:to-emerald-400 rounded-xl transition-all cursor-pointer flex items-center gap-2 shadow-lg shadow-green-500/20 hover:shadow-green-500/40 border border-white/10 flex-shrink-0"
                                           title="Upload ảnh khác">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                                        <span class="hidden md:inline">Đổi ảnh</span>
                                    </label>
                                </div>

                                {{-- Divider 1 (Desktop) --}}
                                <div class="hidden md:block w-px h-8 bg-white/[0.1] order-none md:order-2"></div>

                                {{-- 2. Tools --}}
                                <div class="order-2 md:order-3 flex items-center gap-1 md:gap-1.5 flex-shrink-0">
                                    <button type="button" @click="setTool('brush')" :class="tool === 'brush' ? 'bg-blue-500 text-white shadow-lg shadow-blue-500/25 ring-1 ring-blue-400/50' : 'text-white/70 hover:bg-white/[0.1] hover:text-white'" class="p-2 md:p-2.5 rounded-xl transition-all flex-shrink-0">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                                    </button>
                                    <button type="button" @click="setTool('rect')" :class="tool === 'rect' ? 'bg-blue-500 text-white shadow-lg shadow-blue-500/25 ring-1 ring-blue-400/50' : 'text-white/70 hover:bg-white/[0.1] hover:text-white'" class="p-2 md:p-2.5 rounded-xl transition-all flex-shrink-0">
                                ...
                                {{-- 3. Slider (New Line on Mobile, Inline on Desktop) --}}
                                <div x-show="tool === 'brush'" 
                                     class="order-last md:order-4 w-full md:w-auto flex items-center gap-3 bg-black/20 px-4 py-2 rounded-xl border border-white/[0.05] backdrop-blur-sm transition-all mt-1 md:mt-0">
                                    <span class="text-xs font-medium text-white/60 flex-shrink-0">Size</span>
                                    <input type="range" x-model.number="brushSize" min="5" max="100" class="w-full md:w-28 h-1.5 bg-white/10 rounded-lg appearance-none cursor-pointer accent-blue-500 hover:accent-blue-400">
                                    <span class="text-xs font-mono text-white/80 w-6 text-right flex-shrink-0" x-text="brushSize"></span>
                                </div>

                                {{-- Divider 2 (Desktop) --}}
                                <div class="hidden md:block w-px h-8 bg-white/[0.1] order-none md:order-5"></div>

                                {{-- 4. Actions --}}
                                <div class="order-3 md:order-6 flex items-center gap-1 ml-auto md:ml-0 flex-shrink-0">
                                    <button type="button" @click="undo()" :disabled="historyStep <= 0" class="p-2 md:p-2.5 rounded-xl transition-all text-white/60 hover:bg-white/[0.1] hover:text-white disabled:opacity-30 disabled:cursor-not-allowed">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h10a8 8 0 018 8v2M3 10l6 6m-6-6l6-6" /></svg>
                                    </button>
                                    <button type="button" @click="redo()" :disabled="historyStep >= history.length - 1" class="p-2 md:p-2.5 rounded-xl transition-all text-white/60 hover:bg-white/[0.1] hover:text-white disabled:opacity-30 disabled:cursor-not-allowed">
                                    ...
                                    <div class="hidden md:block w-px h-8 bg-white/[0.1] mx-1"></div>
                                    <button type="button" @click="clearMask()" class="p-2 md:px-4 md:py-2.5 text-red-400 bg-red-500/10 hover:bg-red-500/20 border border-red-500/20 rounded-xl transition-all flex-shrink-0">
                                        <svg class="w-5 h-5 md:w-4 md:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                                        <span class="hidden md:inline ml-2 text-sm font-medium">Xóa Mask</span>
                                    </button>
                                </div>
                            </div>
                        </div>
