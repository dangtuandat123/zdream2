...
<div x-show="showImagePicker" x-cloak x-init="$watch('showImagePicker', v => {
        ...
        })" class="hidden sm:flex fixed inset-0 z-[100] items-center justify-center bg-black/80"
    ...
    <div x-show="showImagePicker" x-transition:enter="transition ease-out duration-300"
        ...
        class="w-full max-w-4xl max-h-[90vh] mx-4 rounded-2xl bg-[#15161A] border border-white/10 overflow-hidden flex flex-col"
        @click.stop>
...
class="sm:hidden fixed inset-0 z-[100] flex items-end justify-center bg-black/80"
...
<div x-show="showImagePicker" x-transition:enter="transition ease-out duration-300"
    ...
    class="w-full max-w-lg bg-[#1a1b20] border-t border-white/10 rounded-t-3xl flex flex-col max-h-[85vh]" @click.stop>
                ...
                <div class="flex flex-wrap gap-2">
                    <template x-for="(img, idx) in selectedImages" :key="img.id">
                        <div class="relative group">
                            <img :src="img.url" class="w-16 h-16 rounded-xl object-cover border-2 border-purple-500/30">
                            <div class="absolute top-1 left-1 w-5 h-5 rounded-full bg-purple-500 text-white text-[10px] font-bold flex items-center justify-center"
                                x-text="idx + 1"></div>
                            <button type="button" @click="removeImage(img.id)"
                                class="absolute -top-1.5 -right-1.5 w-6 h-6 rounded-full bg-red-500 text-white text-xs flex items-center justify-center active:scale-90"><i
                                    class="fa-solid fa-xmark"></i></button>
                        </div>
                    </template>
                </div>
    ...
    <div class="p-4 border-t border-white/5 bg-[#1a1b20] safe-area-bottom shrink-0">
        <button type="button" @click="confirmSelection()"
            ...
            :disabled="selectedImages.length === 0"
            :class="selectedImages.length === 0 ? 'bg-white/10 text-white/40 cursor-not-allowed' : 'bg-gradient-to-r from-purple-600 to-pink-600'">
