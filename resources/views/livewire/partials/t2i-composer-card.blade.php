    ...
    <div class="max-w-4xl mx-auto px-3 sm:px-4 pb-3 sm:pb-4 pt-2 relative">
        ...
        <div class="relative transition-all duration-300 ease-in-out"
            ...
            <div class="relative flex flex-col gap-2.5 t2i-composer-card transition-all duration-500 ease-[cubic-bezier(0.32,0.72,0,1)]"
                :class="!isAtBottom && !isFocused ? 'mx-auto w-full max-w-3xl rounded-2xl p-2 gap-0 bg-black/40 backdrop-blur-[80px] border border-white/10 shadow-2xl' : 'w-full rounded-2xl p-3 sm:p-4 bg-black/60 backdrop-blur-xl border border-white/15 shadow-2xl shadow-black/50'"
                {{-- Prompt textarea --}} <div class="relative flex items-center gap-2 w-full">
                <textarea x-ref="promptInput" wire:model.live.debounce.500ms="prompt" rows="1" @focus="isFocused = true"
                    @blur="isFocused = false" placeholder="Mô tả ý tưởng của bạn..."
                ...
                <button type="button" x-show="!isAtBottom && !isFocused"
                    ...
                    x-transition:enter-end="opacity-100 scale-100" @click="$wire.generate()"
                    :disabled="$wire.prompt?.length === 0"
                                    ...
                                    <div class="space-y-1">
                                        <template x-for="model in models" :key="model.id">
                                            <button type="button" @click="selectModel(model.id)"
                                                ...
                                                <div class="flex-1 min-w-0">
                                                    <div class="text-white font-semibold text-base" x-text="model.name">
                                                    </div>
